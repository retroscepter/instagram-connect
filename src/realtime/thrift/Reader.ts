...
function getReadFunction(
    ...
    switch (type) {
        ...
        case ThriftTypes.STOP: {
            throw new Error(`Invalid state: got type ${type}`)
        }
...
export function thriftReadToObject<T>(
    message: Buffer,
    ...
    const result: ThriftToObjectResult<T> = thriftReadSingleLevel(
        topLevel,
        descriptors
    )
    ...
    for (const readData of readResult) {
        ...
        const possible = structs.findIndex((s) =>
            equalArrays(s.fieldPath, fieldPath)
        )
        if (possible !== -1) {
    ...
    for (const struct of structs) {
        ...
        for (const level of struct.fieldPath) {
            if (descriptor) {
                descriptor = descriptor.structDescriptors?.find(
                    (x) => x.field === level
                )
            } else {
        ...
        if (descriptor) {
            defineEnumerableProperty(
                ...
                thriftReadSingleLevel(
                    struct.items,
                    descriptor.structDescriptors ?? []
                )
            )
        } else {
    ...
    for (const message of readResults) {
        const descriptor = descriptors.find(
            (d) =>
                ...
                (d.type === message.type ||
                    (isThriftBoolean(message.type) && isThriftBoolean(d.type)))
        )
        if (descriptor) {
...
export class BufferReader {
    ...
    private move(bytes: number) {
        this._position = Math.min(
            Math.max(this._position + bytes, 0),
            this.buffer.length
        )
    ...
    public readString = (len: number): string =>
        this.buffer.toString('utf8', this.move(len), this._position)
