...
export function thriftWriteFromObject(
        ...
        switch (descriptor.type & 0xff) {
            ...
            case ThriftTypes.INT_64: {
                if (typeof value === 'number') {
                    ...
                    throw new Error(
                        `Value of ${name} is neither a bigint nor a number`
                    )
                }
                break
            }
            ...
            case ThriftTypes.STRUCT: {
                ...
                thriftWriteSingleLayerFromObject(
                    value,
                ...
                if (descriptor.type === ThriftTypes.MAP_BINARY_BINARY) {
                    ...
                    if (pairs.length !== 0) {
                        for (const pair of pairs) {
                            writer
                                .writeStringDirect(pair[0])
                                .writeStringDirect(pair[1])
                        }
                    }
                } else {
            ...
            default: {
                throw new Error(
                    `Could not find type ${descriptor.type} for ${name}`
                )
            }
        }
    }
...
export class BufferWriter {
    ...
    public writeBoolean(field: number, bool: boolean): this {
        return this.writeField(
            field,
            bool ? ThriftTypes.TRUE : ThriftTypes.FALSE
        )
    }
    ...
    public writeList(field: number, type: number, list: []): this {
        ...
        switch (type) {
            ...
            case ThriftTypes.FALSE: {
                list.forEach((el) =>
                    this.writeByte(el ? ThriftTypes.TRUE : ThriftTypes.FALSE)
                )
                break
            }
            case ThriftTypes.BYTE: {
